...
export const DeleteForm = ({
  ...
  const deleteRows = async (event) => {
    ...
    selectedRows.forEach((removeId) => {
      for (let row in data) {
        ...
        if (
          removeId === data[row].walletId &&
          data[row].accountType === "interfacility"
        ) {
  ...
  const finalizeDeleteInitiator = async () => {
    ...
    if (status.success) {
      ...
      alert(
        `Error removing ${Constants.POLICE} from DB, please contact SuperAdmin`
      );
    }
      ...
      {(function () {
        if (showMessage2) {
            ...
            return (
              <div>
                <Alert severity="warning">
                  Waiting for {Constants.POLICE}(s) to be deleted
                </Alert>
                <Box sx={{ display: "flex" }}>
                  <CircularProgress />
                </Box>
          ...
          if (transactionErrored(state) && !deleteInterfacility) {
            ...
            (transactionErrored(state3) || transactionErrored(state)) &&
            deleteInterfacility
          ) {
          ...
          if (
            ...
            return (
              <div>
                <Alert severity="success">
                  Your transaction was successful! {Constants.POLICE}(s) were
                  deleted from the blockchain
                </Alert>
                <Button color="success" onClick={finalizeDeleteInitiator}>
                  Finalize and Exit
                </Button>
              </div>
            );
            ...
            return (
              <div>
                <Alert severity="success">
                  Your transaction was successful! Interfacility account was
                  deleted from the blockchain
                </Alert>
                <Button color="success" onClick={finalizeDeleteInitiator}>
                  Finalize and Exit
