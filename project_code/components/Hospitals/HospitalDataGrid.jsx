...
export default function HospitalDataGrid({
  ...
  const {
    ...
    send: send1,
    events: events1,
  } = useContractFunction(ACCOUNT_INSTANCE, "addHospital");
  const {
    state: state2,
    send: send2,
    events: events2,
  } = useContractFunction(ACCOUNT_INSTANCE, "removeHospital");
  const {
    state: state3,
    send: send3,
    events: events3,
  ...
  const finalizeAddHospital = async () => {
    ...
    if (status.success) {
      ...
      alert(
        `Error adding ${Constants.HOSPITAL} to DB, please contact SuperAdmin`
      );
    }
  };
  ...
  const deleteRows = async (event) => {
    ...
    selectedRows.forEach((removeId) => {
      for (let row in dataContacts) {
        ...
        if (
          removeId === dataContacts[row].walletId &&
          dataContacts[row].accountType === "interfacility"
        ) {
  ...
  const finalizeDeleteHospital = async () => {
    ...
    if (status.success) {
      ...
      alert(
        `Error removing ${Constants.HOSPITAL} from DB, please contact SuperAdmin`
      );
    }
          ...
          return (
            <div>
                ...
                {(function () {
                  if (showMessage2) {
                    if (state2.status === "Mining" && !deleteInterfacility) {
                      return (
                        <div>
                          <Alert severity="warning">
                            Waiting for {Constants.HOSPITAL}(s) to be deleted
                          </Alert>
                          <Box sx={{ display: "flex" }}>
                            <CircularProgress />
                          </Box>
                      ...
                      state2.status === "Mining" &&
                      state3.status === "Mining"
                    ) {
                      return (
                        <div>
                          <Alert severity="warning">
                            Waiting for Interfacility to be deleted
                          </Alert>
                          <Box sx={{ display: "flex" }}>
                            <CircularProgress />
                          </Box>
                    ...
                    if (transactionErrored(state2) && !deleteInterfacility) {
                      ...
                      (transactionErrored(state3) ||
                        transactionErrored(state2)) &&
                      deleteInterfacility
                    ) {
                      if (transactionErrored(state3)) {
                        return (
                          <div>
                            <Alert severity="error">
                              Transaction failed: {state3.errorMessage}
                            </Alert>
                          </div>
                        );
                    ...
                    if (
                      ...
                      return (
                        <div>
                          <Alert severity="success">
                            ...
                            {Constants.HOSPITAL} was deleted from the blockchain
                          </Alert>
                          <Button
                            color="success"
                            onClick={finalizeDeleteHospital}
                          >
                            Finalize and Exit
                          </Button>
                        </div>
                      ...
                      return (
                        <div>
                          <Alert severity="success">
                            Your transaction was successful! Interfacility
                            account was deleted from the blockchain
                          </Alert>
                          <Button
                            color="success"
                            onClick={finalizeDeleteHospital}
              ...
              <Popup trigger={addPopup} setTrigger={setAddPopup}>
                ...
                <Alert className={stylesP.warningText} severity="warning">
                  NOTE: If Wallet ID is already registered as an{" "}
                  {Constants.POLICE} the account will become interfacility
                </Alert>
                <form
                  ...
                  if (showMessage1) {
                    if (state1.status === "Mining") {
                      return (
                        <div>
                          <Alert severity="warning">
                            Waiting for {Constants.HOSPITAL} to be added
                          </Alert>
                          <Box sx={{ display: "flex" }}>
                            <CircularProgress />
                          </Box>
                    ...
                    if (state1.status === "Success" && events1 != undefined) {
                      return (
                        <div>
                          <Alert severity="success">
                            Your transaction was successful!{" "}
                            {Constants.HOSPITAL} was added to the blockchain
                          </Alert>
                          <Button color="success" onClick={finalizeAddHospital}>
                            Finalize and Exit
                          </Button>
                        </div>
                      );
