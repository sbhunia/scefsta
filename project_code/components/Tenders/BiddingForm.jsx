    ...
    <div className={styles.editHospital}>
      ...
      <div>
        <FormControl fullWidth variant="filled" className={styles.givenRewardDiv}>
          <InputLabel htmlFor="filled-adornment-amount">Enter your bid value:</InputLabel>
          <FilledInput
      ...
      <div className={styles.buttonDiv}>
        ...
        {(function () {
          if (state1.status === 'Mining') {
            return (
              <label>Waiting for bid to be placed
              <Box sx={{ display: 'flex' }}>
                <CircularProgress />
              </Box>
              </label>
          ...
          if (state1.status === 'Success' && events != undefined) {
            return (
              <div>
                <Alert severity="success">Your transaction was successful! Your bid ID is: {bidID}</Alert>
